...
func TestHNSW_SingleRef(t *testing.T) {
	s := ann.New([]ann.Reference{{Vector: zeroVec, Label: "legit"}}, zap.NewNop())
...
func TestHNSW_ReturnsExactlyKLabels(t *testing.T) {
	refs := make([]ann.Reference, 10)
	for i := range refs {
		refs[i] = ann.Reference{Vector: zeroVec, Label: "legit"}
	}
	s := ann.New(refs, zap.NewNop())
...
func TestHNSW_Top5AreFraud(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		...
		refs[i] = ann.Reference{Vector: v, Label: "fraud"}
	}
	for i := 5; i < 10; i++ {
		var v domain.Vector
		for j := range v {
		...
		refs[i] = ann.Reference{Vector: v, Label: "legit"}
	}
	s := ann.New(refs, zap.NewNop())
